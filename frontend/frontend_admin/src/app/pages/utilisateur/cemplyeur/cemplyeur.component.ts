...
@Component({
  selector: 'app-cemplyeur',
  templateUrl: './cemplyeur.component.html',
  styleUrls: ['./cemplyeur.component.css']
})
export class CemplyeurComponent implements OnInit {
    ...
    if (this.authService.isLoggedIn()) {
      this.utilisateurService.getByEmail(this.currentUser).subscribe((data: Utilisateur) => {
        ...
        if(this.userService.rolesMatch(['CHEF DAPRATEMENT'])){
          this.utilisateurService.byDepEmployee(this.utilisateur.departement.id).subscribe((data: Employee[]) => {
      ...
      if(this.userService.rolesMatch(['ADMIN'])){
        this.utilisateurService.getAllEmp().subscribe((data: Employee[]) => {
          console.log(data)
          this.listemp = data
        })
      } 
  ...
  ajouterEmpl(form: NgForm) {
    console.log(form.value)
    if (form.valid) {

    const formData = new FormData();
    formData.append('emp', new Blob([JSON.stringify(this.user)], { type: 'application/json' }));
    formData.append('id', this.utilisateur.departement.id.toString());

    this.utilisateurService.saveEmployeur(formData).subscribe((data: Employee) => {
      if (data != null) {
  ...
  updateEmpl(form: NgForm) {
    ...
    this.utilisateurService.updateEmployee(this.user.id, this.user).subscribe((data: Employee) => {
      if (data != null) {
        alert("Employeur mis à jour avec succès");
        this.router.navigate(['/employee']).then(() => {
          location.reload();
        });
      } else {
  ...
  update(id: number) {
    ...
    active(id: number) {
      this.utilisateurService.activer(id).subscribe((data:Utilisateur)=>{
        console.log(data)
        if(data.etat=false){
          alert("Desactivation avec succés ")
          this.router.navigate(['/employee']).then(()=>{
            location.reload()
          })
        }else if(data.etat=true){
          alert("Activation avec succés")
          this.router.navigate(['/employee']).then(()=>{
            location.reload()
          })
        }
  ...
  reset(form: NgForm) {
    ...
    filteredListemp(): Employee[] {
      return this.listemp.filter(emp =>
