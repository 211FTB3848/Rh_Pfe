...
export class OfferEmploiService {
  ...
  getOfferByDep(id: number): Observable<OfferEmploi[]> {
    return this.http.get<OfferEmploi[]>(`${this.baseUrl}/findOfferByDep/${id}`);
  }
  ...
  saveOffer(offerEmploi: OfferEmploi, idEmp: number): Observable<OfferEmploi> {
    return this.http.post<OfferEmploi>(`${this.baseUrl}/save?idEmp=${idEmp}`, offerEmploi);
  }
